...
class DeeplabLarge(nn.Module):
    def __init__(self, cfg: dict):
        ...
        self.iou = JaccardIndex(
            num_classes=self.n_classes, reduction="none", ignore_index=0
        )
        self.ap = MeanAveragePrecision(
            box_format="xyxy", num_classes=self.n_classes, reduction="none"
        )
        ...
        self.network = torchvision.models.detection.maskrcnn_resnet50_fpn(
            weights=None, progress=True, num_classes=self.n_classes
        )
    ...
    def validation_step(self, batch):
        ...
        for b_idx in range(b):
            sem_gt = (
                batch["targets"][b_idx]["labels"].unsqueeze(dim=1).unsqueeze(dim=1)
                * batch["targets"][b_idx]["masks"]
            )
            sem_gt, _ = sem_gt.max(dim=0)

            sem_out = (
                out[b_idx]["labels"].unsqueeze(dim=1).unsqueeze(dim=1)
                * out[b_idx]["masks"]
            )
            sem_out, _ = sem_out.max(dim=0)

            ins_out = (
                torch.arange(out[b_idx]["masks"].shape[0])
                .unsqueeze(dim=1)
                .unsqueeze(dim=1)
                * out[b_idx]["masks"]
            )
